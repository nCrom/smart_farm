...
### 2025-03-03: Docker 환경 설정 및 라이믹스 CMS 설치

1. **Docker 환경 설정**:
   - Docker Compose를 사용하여 다음 컨테이너 설정:
     - PHP 웹 서버 (smart_farm)
     - MariaDB 데이터베이스 (smart_farm_db)
     - phpMyAdmin (smart_farm_phpmyadmin)

2. **라이믹스 CMS 설치**:
   - `install_rhymix.php` 스크립트 작성 및 활용
   - 설치 과정 중 데이터베이스 연결 문제 해결
   - 기본 테마 및 설정 적용

3. **GitHub 통합**:
   - 프로젝트 `.gitignore` 파일 업데이트 (라이믹스 전용 설정 추가)
   - 설치 완료 후 설정 파일 및 중요 데이터 보호 설정
